...
def json_dump_01(data, comment=None, serializer=None):
  if comment:
...
def json_load_01(json_str, comment=None, deserializer=None):
  if comment:
    logger.debug(f"{comment}\n")
...
if __name__ == "__main__":
  ...
  json_str = json_dump_01(data_list, comment='Array of multiple classes', serializer=custom_serializer)
  # json_str = json_dump_01(data_dict, comment='Dict of multiple classes', serializer=custom_serializer)

  # Deserialize
  decoded_data = json_load_01(json_str, deserializer=deserializer)
