...
def get_sa_column_types(model, is_instance: bool = True) -> dict[str, dict]:
  ...
  """
  inspector = inspect(type(model)) if is_instance else inspect(model)
  logger.debug(f"{model=}\n{inspector=}")
...
def get_sa_automap_types(engine, base) -> dict:
  """
  Return column type info for all automapped tables.

  Args:
      engine: SQLAlchemy Engine.
      base: Automap base object (e.g., automap_base()).

  Returns:
      dict: Nested dictionary of type info:
          result[table][column] = {
              "database_type": ...,
              "sqlalchemy_type": ...,
              "python_type": ...
  ...
  for class_name, mapped_class in base.classes.items():
    ...
    db_columns = inspector.get_columns(class_name)

    for column in columns:
      col_result = {}
      db_type = next((col["type"] for col in db_columns if col["name"] == column.name), None)
      sa_type = type(db_type).__name__ if db_type else None

      try:
        py_type = column.type.python_type
      except Exception as e:
        logger.warning(f"{column.name} type={sa_type} lacks python_type")
        logger.warning(e)
        py_type = None

      col_result["database_type"] = db_type
      col_result["sqlalchemy_type"] = sa_type
      col_result["python_type"] = py_type

      result[class_name][column.name] = col_result
