...
New Feature:
  - Tracks dropdown list changes (data validation of type 'list')
    * Detects added, removed, or modified data validation settings
    * Compares formula1, formula2, allow_blank, and showDropDown
...
def compare_excel_content(path1: Path, path2: Path, formatting_mode: str = "common") -> list[str]:
  """
  Compare cell content, formatting, formulas, comments, and data validation.

  Args:
    path1 (Path): Path to first Excel file.
    path2 (Path): Path to second Excel file.
    formatting_mode (str): 'off', 'common', or 'full' for formatting comparisons.

  Returns:
    list[str]: Formatted report lines describing all detected differences.
  """
  ...
  for sheet in in_both:
    ...
    comment_diffs, formatting_diffs, validation_diffs = [], [], []
