"""
Legacy hardcoded schema definitions and sample payloads for Excel template processing.

These were manually created by inspecting v01 and v02 versions of the Excel spreadsheets.
This module is still used to support legacy conversion and comparison workflows but is no longer updated.

Contents:
    - `default_value_types_v01_00`: field types for v01/v02 schema
...
jinja_names_set = {
    "additional_activities", "additional_notes", "component_at_source", "component_other_description",
    "contact_email", "contact_name", "contact_phone", "equipment_at_source", "equipment_other_description",
    "facility_name", "final_repair_concentration", "id_arb_eggrt", "id_arb_swis", "id_incidence", "id_message",
    "id_plume", "included_in_last_lmr", "included_in_last_lmr_description", "initial_leak_concentration",
    "initial_mitigation_plan", "inspection_timestamp", "instrument", "last_component_leak_monitoring_timestamp",
    "last_surface_monitoring_timestamp", "lat_carb", "lat_revised", "long_carb", "long_revised",
    "method21_date", "method21_performed", "method21_result", "mitigation_actions", "mitigation_timestamp",
    "observation_timestamp", "ogi_date", "ogi_performed", "ogi_result", "planned_for_next_lmr",
    "planned_for_next_lmr_description", "re_monitored_concentration", "re_monitored_timestamp",
    "repair_description", "repair_timestamp", "venting_description_1", "venting_description_2",
    "venting_exclusion"
}

# -------------------------------------------------------------------------------------
# Diagnostic: Compare legacy schema to jinja template fields
# -------------------------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    in_schema_only, in_jinja_only = list_differences(
        default_value_types_v01_00, jinja_names,
        iterable_01_name="Legacy Schema",
        iterable_02_name="Jinja Fields",
        print_warning=False
    )

    print(f"{in_schema_only=}")
    print(f"{in_jinja_only=}")
