...
def get_sector_info(db, base, id_: int) -> tuple[str, str]:
  ...
  Args:
      db: SQLAlchemy database associated with a Flask app.
      base: SQLAlchemy declarative base.
      id_: Primary key of the 'incidences' table.

  Returns:
      tuple: (sector, sector_type)
...
def resolve_sector_type(row, misc_json: dict) -> tuple[str, str]:
  ...
  if "sector_type" not in misc_json:
    ...
    logger.warning(f"Sector Type mismatch; JSON will not be adjusted from {misc_json['sector_type']} to {sector_type=}")
...
def resolve_sector(sector_by_foreign_key: str | None, row, misc_json: dict) -> str:
  ...
  Args:
      sector_by_foreign_key: Sector as determined via foreign key lookup.
      row: SQLAlchemy model instance for the current row.
      misc_json: JSON column data.

  Returns:
      str: Sector value after resolving mismatches.
  """
...
def upload_and_update_db(db, upload_dir, request_file, base) -> tuple[str, int | None, str | None]:
  ...
  Args:
      ...
      request_file: werkzeug.datastructures.FileStorage object from request.files['file'].
      base: SQLAlchemy declarative base.

  Returns:
      tuple: (file_name, id_incidence, sector)
  """
...
def xl_dict_to_database(db, base, xl_dict: dict, tab_name: str = "Feedback Form") -> tuple[int, str]:
  ...
  Args:
      db: SQLAlchemy DB instance.
      base: SQLAlchemy declarative base.
      xl_dict: Excel-formatted JSON dict.
      tab_name: Name of worksheet/tab to load from.
...
# todo - check this closely for logic issues
def dict_to_database(db,
  ...
  Args:
      db: SQLAlchemy DB instance.
      base: SQLAlchemy declarative base.
      data_dict: Dictionary of data for insertion.
      table_name: Name of target table.
      json_field: Name of JSON column.
  ...
  # todo (cosider) - use the payload routine submit_and_log_model
  ...
  if model_json_dict.get("sector") in ["Oil and Gas", "Oil & Gas"] and not model_json_dict.get("id_plume"):
    model_json_dict["id_plume"] = 123456  # placeholder for future lookup
...
def add_file_to_upload_table(db, file_name, status: str | None = None, description: str | None = None) -> None:
  """
