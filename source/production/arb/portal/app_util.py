...
def dict_to_database(db, base, data_dict, table_name="incidences", json_field="misc_json"):
  ...
  Notes:
  - If data_dict has an id_incidence that is in the DB, the row is updated.
  ...
  id_ = data_dict.get("id_incidence")
  new_row = False
  ...
  # Back-fill id_incidence into the misc_json column if it was newly generated
  if new_row and model.id_incidence is not None:
    model.misc_json["id_incidence"] = model.id_incidence
    flag_modified(model, json_field)
    session.commit()

  return model.id_incidence
