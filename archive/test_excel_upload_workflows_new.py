...
class TestExcelUpload:
    ...
    def test_file_upload_workflow(self, upload_page: Page, file_path: str):
        ...
        expect(
            upload_page.locator(".alert-success, .alert-danger, .alert-warning, .success-message, .error-message")
        ).to_have_count_greater_than(0, timeout=10000)
    ...
    def test_large_file_upload(self, upload_page: Page):
        ...
        expect(
            upload_page.locator(".alert-success, .alert-danger, .alert-warning, .success-message, .error-message")
        ).to_have_count_greater_than(0, timeout=10000)  # Wait for result message to appear
...
def test_list_staged_diagnostics_overlay(page):
    ...
    if staged_file_btns.count() == 0:
        ...
        expect(
            page.locator(".alert-success, .alert-danger, .alert-warning, .success-message, .error-message")
        ).to_have_count_greater_than(0, timeout=10000)
