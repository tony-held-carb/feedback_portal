...
def ensure_placeholder_option(
  ...
  """
  Add or move a placeholder option to the top of a selector list.

  Args:
      tuple_list (list): List of selector tuples.
      item (str): Placeholder key (default: "Please Select").
      item_dict (dict | None): Metadata dictionary (default: {"disabled": True}).
      ensure_first (bool): Move to top if found but not first.

  Returns:
      list: Modified tuple list.

  Example:
      >>> ensure_placeholder_option([("B", "B", {})])
      [('Please Select', 'Please Select', {'disabled': True}), ('B', 'B', {})]
  """
...
if __name__ == '__main__':
  ...
  now_string = "Not a valid datetime"
  new_now = str_to_datetime(now_string)
  print(f"{new_now=}")
