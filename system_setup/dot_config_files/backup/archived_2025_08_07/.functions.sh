...
# detect_os
#
# Determines the operating system context and sets OS_TYPE to one of:
#   - "wsl"     → WSL (Windows Subsystem for Linux)
#   - "windows" → Native Windows (Git Bash, CMD, PowerShell)
#   - "linux"   → Native Linux (EC2, VMs, physical Linux)
#
# This function is intended for use in interactive shells (.bashrc) and
# should be run at startup to help configure environment-specific logic.
#
# Detection logic:
#   1. Check for WSL by inspecting /proc/version (contains "Microsoft" in WSL).
#   2. If not WSL:
#       - If running in Bash AND $OS is set to "Windows_NT", assume Windows (Git Bash).
#       - Otherwise, assume native Linux.
#
# Usage:
#   Place this function in your ~/.bashrc or ~/.profile and call it at startup.
# -----------------------------------------------------------------------------
...
#
# Determines the physical or virtual location of the machine and sets
# MACHINE_LOCATION to one of the following:
#
#   - "TONY_HOME"  → if NAME == "tonydesktop" OR HOSTNAME == "tony_desktop"
#   - "TONY_WORK"  → if HOSTNAME == "5CG3251H1W"
#   - "TONY_EC2"   → if HOSTNAME == "ip-10-93-112-44.ad.arb.ca.gov"
#   - "UNKNOWN"    → if none of the above match
#
# This function should be placed in .bashrc and executed during shell startup.
# -----------------------------------------------------------------------------
...
detect_machine_location() {
  ...
  elif [ "$HOSTNAME" = "ip-10-93-112-44.ad.arb.ca.gov" ]; then
    export MACHINE_LOCATION="TONY_EC2"
  else
    export MACHINE_LOCATION="UNKNOWN"
  fi
}
